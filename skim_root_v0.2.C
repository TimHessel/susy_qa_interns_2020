...
void skim_root(){
  ...
  const char * pres = preselection.c_str();
  ...
  for (i = 0; i < 1; i++){
    ...
    //Getting the tree from the root file
    TFile oldfile(c1, "READ");
    TTree* full_tree = static_cast <TTree*>(oldfile.Get("bdttree"));

    //Copying the tree to a temp with preselection
    TTree* tmp_tree = full_tree->CopyTree(pres);

    //Setting only the needed branches to active
    ...
    //Naming the out root file
    string file_out = ("skimmed_" + data_bkg[i]);
    const char * c3 = file_out.c_str();
    TFile newfile(c3, "RECREATE");

    // Cloning the tmp tree to a final tree and write
    TTree* skimmed_tree = tmp_tree->CloneTree();
    skimmed_tree->Write();
  }
  int k = 0;
  for (k = 0; k < 26; k++){
    ...
    //Copying the tree to a temp with preselection
    TTree* tmp_tree = full_tree->CopyTree(preselection);
    ...
    //Naming the out root file
    string file_out = ("skimmed_" + data_sig[i]);
    const char * c3 = file_out.c_str();
    TFile newfile(c3, "RECREATE");
