/* 
Scratched version for extraction of root file data.
Selecting only wanted branches without preselection.
10/05/2020
*/
#include <iostream>
#include <iomanip>
...
void skimming_rootfile(string data[], string path, string branches[], const char* dir){
  ...
  for (i = 0; i < n_data; i++){
    ...
    string file_out = (string(dir) + "trimmed_" + data[i]);
    const char * c3 = file_out.c_str();
    TFile newfile(c3, "RECREATE");
    TTree* branched_tree = full_tree->CloneTree();
    branched_tree->Write();
  }
}

void branches_rootfile(){
  //Path to data
  ...
  const char* directory_name = "branched_data/";
